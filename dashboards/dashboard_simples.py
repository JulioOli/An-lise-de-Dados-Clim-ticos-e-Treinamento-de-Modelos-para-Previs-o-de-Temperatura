...
with col1:
    btn_overview = st.button("📊 Visão Geral", use_container_width=True)
with col2:
    btn_models = st.button("🤖 Modelos ML", use_container_width=True)
with col3:
    btn_timeseries = st.button("📈 Séries Temporais", use_container_width=True)
with col4:
    btn_correlation = st.button("🔗 Correlação", use_container_width=True)
with col5:
    btn_distributions = st.button("📊 Distribuições", use_container_width=True)
with col6:
    btn_scatter = st.button("📉 Scatter Plots", use_container_width=True)
with col7:
    btn_data = st.button("📋 Dados", use_container_width=True)
...
# ==================== SEÇÃO: MODELOS ML ====================
    ...
    if comparison_df is not None:
        ...
        fig_models = px.bar(comparison_df, x='Modelo', y='R² Score',
                           title='Comparação R² Score dos Modelos',
                           color='R² Score',
        ...
        with col1:
            fig_mse = px.bar(comparison_df, x='Modelo', y='MSE',
                            title='Mean Squared Error por Modelo',
                            color='MSE',
                            color_continuous_scale='Reds_r')
            st.plotly_chart(fig_mse, use_container_width=True)
        
        with col2:
            fig_mae = px.bar(comparison_df, x='Modelo', y='MAE',
                            title='Mean Absolute Error por Modelo',
                            color='MAE',
                            color_continuous_scale='Reds_r')
...
# ==================== SEÇÃO: SÉRIES TEMPORAIS ====================
    ...
    fig_sazonal = px.line(dados_mensais, x='mes', y=variavel,
                         ...
                         markers=True)
    fig_sazonal.update_xaxis(tickmode='array', tickvals=list(range(1, 13)),
...
# ==================== SEÇÃO: DISTRIBUIÇÕES ====================
    ...
    with col2:
        ...
        st.plotly_chart(fig_hist_press, use_container_width=True)
...
# ==================== SEÇÃO: DADOS BRUTOS ====================
    ...
    st.dataframe(dados_mostrar, use_container_width=True)
    
    # Estatísticas descritivas
    st.subheader("📊 Estatísticas Descritivas")
    st.dataframe(dados_lag.describe(), use_container_width=True)
